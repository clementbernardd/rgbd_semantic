...
def plot_multiple_images(images_dict, rows = 1 , cols = 1, figsize=(20,8) ):
    ''' Plot the images from the dictionnary : {title : image} '''
    figure, ax = plt.subplots(figsize = figsize , nrows=rows,ncols=cols )
    ax = np.array(ax)
...
def plot_results(scores_train, scores_val) :
  ''' Plot the scores for the validation and training sets'''
  epochs = []
  pixel_acc = {'train' : [], 'val' : []}
  mean_acc = {'train' : [], 'val' : []}
  mean_iou = {'train' : [], 'val' : []}

  for e in scores_train :
    epochs.append(e)
    pixel_acc['train'].append(scores_train[e]['Pixel_accuracy'])
    pixel_acc['val'].append(scores_val[e]['Pixel_accuracy'])
    mean_acc['train'].append(scores_train[e]['Mean_accuracy'])
    mean_acc['val'].append(scores_val[e]['Mean_accuracy'])
    mean_iou['train'].append(scores_train[e]['Mean_iou'])
    mean_iou['val'].append(scores_val[e]['Mean_iou'])

  figure, ax = plt.subplots( figsize=(20,8),  nrows=1,ncols=3)
  ax = np.array(ax)

  to_plot = {'Pixel accuracy' : pixel_acc, 'Mean accuracy' : mean_acc , 'Mean IoU' : mean_iou}

  for i,title in enumerate(to_plot) :
      ax.flatten()[i].plot(epochs, to_plot[title]['train'] , label = 'Training set', marker = '+')
      ax.flatten()[i].plot(epochs, to_plot[title]['val'] , label = 'Validation set', marker = '+')
      ax.flatten()[i].set_title(title)
...
def compare_predictions(y_pred, y, n = 4 , rows = 2 , cols = 2) :
  ''' Plot few examples '''
  name_y_pred = 'Prediction for Image '
  name_y = 'Label for Image '
  comparison = {}
  for i in range(n) :
    comparison[name_y_pred + str(i)]  =  y_pred[i]
    comparison[name_y + str(i)]  =  y[i]

  plot_multiple_images(comparison, rows, cols)
...
def plot_final_results(data) :
    ''' Plot the distribution of results from the dataframe '''

    all_distribution = []
    for model in data :
        all_distribution.append({'Model' : model,
                                'Score' : data[model][1],
                                'marker' : 'Mean IoU'})
        all_distribution.append({'Model' : model,
                                'Score' : data[model][0],
                                'marker' : 'Mean accuracy'})
        all_distribution.append({'Model' : model,
                                'Score' : data[model][2],
                                'marker' : 'Pixel Accuracy'})
    df = pd.DataFrame(all_distribution)

    # Plot the results
    sns.set(rc={'figure.figsize':(14,20)}, font_scale=2.1)
    g = sns.catplot(x="Model", y="Score", hue="marker", data=df,
                    kind="bar", height=12, aspect=2)


    g.set_xticklabels(g.ax.get_xticklabels(), rotation=90, ha="center", fontsize = 30)
...
def compare_model_example(X_test , y_test , fusenet, gcn) :
    ''' Plot the predictions for fusenet and gcn '''
    images = {}
    X = [X_test[0][:6], X_test[1][:6]]
    y = y_test[:6]

    fusenet_pred = fusenet.predict(X)
    gcn_pred = gcn.predict(X)

    for i in range(6) :
        images['RGB '+str(i)] = X[0][i].cpu().reshape(240,320,3).cpu().detach().numpy().astype(np.uint8)
        images['Depth '+str(i)] = X[1][i].cpu().reshape(240,320,1).cpu().detach().numpy().astype(np.uint8)
        images['Label '+str(i)] = y[i].cpu().reshape(240,320,1).cpu().detach().numpy().astype(np.uint8)
        images['FuseNet '+str(i)] = fusenet_pred[i].reshape(240,320,1).astype(np.uint8)
        images['Gcn '+str(i)] = gcn_pred[i].reshape(240,320,1).astype(np.uint8)

    plot_multiple_images(images, rows = 6 , cols = 5, figsize=(250,130) )
